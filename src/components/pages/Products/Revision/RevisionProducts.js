    ...
    Axios.get("/api/revision/unitsbybarcode", {
      ...
      .then((products) => {
        if (products.length > 0) {
          ...
          Alert.warning(
            `Товар со штрих-кодом ${barcode} отсутствует на складе ${
              type === 2
                ? `под брендом "${object.label}"`
                : type === 3
                ? `под категорией "${object.label}"`
                : ""
            }`,
            {
              position: "top-right",
  ...
  return (
    <Fragment>
      {fewProducts.length > 0 && (
        <ReactModal isOpen={isSelectProduct} style={customStyles}>
          <Grid container>
          ...
          <Grid container spacing={2}>
            ...
            {fewProducts.map((prod, idx) => (
              <Fragment key={idx}>
                <Grid item xs={8}>
                  {prod.attrvalue ? prod.attrvalue : "Без характеристик"}
                </Grid>
        ...
        <Grid container wrap="nowrap" spacing={2}>
          ...
          <Grid item xs={6}>
            <button
              ...
              className="btn btn-success"
              disabled={
                revisionProducts.length > 0 && admin === user ? false : true
              }
            >
              Далее
            </button>
